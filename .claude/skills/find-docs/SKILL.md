...
description: >-
  ...
  developer technology, via the Context7 CLI. Each lookup costs tokens, so use
  it sparingly — prefer answering from training knowledge by default.

  Use only when: the user explicitly asks to check/verify against documentation
  (mentions "Context7", "ctx7", "docs atualizados", etc.); you're about to write
  code against an API you're genuinely unsure about (unfamiliar library, a
  version well past training cutoff, or a suspected recent breaking change); or
  a bug looks like a version-specific behavior change that training knowledge
  isn't resolving.

  Do NOT use for routine mentions of a library name, or for APIs/patterns you
  already know confidently — only genuine uncertainty or an explicit user
  request should trigger this.
---
