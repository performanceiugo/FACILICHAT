...
| 4 | Dashboard do gestor (web) | `868k60w16` | ✅ Concluída · backend, frontend e manutenção operacional (categorias/equipe) fechados em 15/07/2026; validação visual do usuário ainda pendente para Categorias/Equipe/sidebar |
...
| Status | CU | Item | Arquivo(s) |
|--------|----|------|-----------|
| `[x]` | `868kcw5xj` | Página `/painel/categorias` — listar categorias da Empresa, criar, editar nome e ativar/desativar. Validado em 15/07/2026: TypeScript sem erros; Playwright cobriu criar/editar nome/ativar/desativar sem erro de console (screenshots revisados pelo agente — ainda sem revisão visual do usuário) | `frontend/web/src/app/painel/categorias/page.tsx` (novo), `categorias.module.css` (novo), `frontend/web/src/lib/api.ts`, `frontend/web/src/types/index.ts` |
| `[x]` | `868kcw5z1` | Página `/painel/equipe` — lista a equipe da Empresa, cria (reaproveita `POST /usuarios/equipe`), edita dados cadastrais e ativa/desativa (inclui adicionar/remover Supervisor); tela separada do dashboard de métricas em `/painel/supervisores` (que continua só leitura). Validado em 15/07/2026: TypeScript/lint sem novidade, Playwright cobriu contratar/editar/ativar-desativar e confirmou a trava de autodesativação do Gestor (mensagem inline), sem erro de console além do 400 esperado (screenshots revisados pelo agente — ainda sem revisão visual do usuário) | `frontend/web/src/app/painel/equipe/page.tsx` (novo), `equipe.module.css` (novo), `frontend/web/src/lib/api.ts`, `frontend/web/src/types/index.ts` |
| `[x]` | `868kcw619` | Adicionar links no sidebar: Categorias e Equipe. Validado em 15/07/2026: TypeScript sem erros, Playwright confirmou navegação e destaque `aria-current` para os dois links, sem erro de console (screenshots revisados pelo agente — ainda sem revisão visual do usuário) | `frontend/web/src/components/painel/AdminShell.tsx` |
